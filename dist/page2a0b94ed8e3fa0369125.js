...
var assignListenerToBoard = function assignListenerToBoard(event, board) {
  ...
  boardElements.forEach(function (element, index) {
    ...
    element.addEventListener('click', element.fn = function (e) {
      var positionStatus = assignPositionAndDeleteInteraction(e, board, numberID, boardElements);
      positionStatus ? _interface__WEBPACK_IMPORTED_MODULE_0__.shipLocationOnBoard(index, event.target.dataset.length) : console.log('FAILED');
    }, {
      once: true
    });
  });
...
var assignPositionAndDeleteInteraction = function assignPositionAndDeleteInteraction(event, board, numberID) {
  var assignStatus = board.assignShipPosition(numberID, Number(event.target.dataset.x), Number(event.target.dataset.y));
  assignStatus ? _interface__WEBPACK_IMPORTED_MODULE_0__.deleteBoardInteraction() : false;
};
...
/******/ })()
;
//# sourceMappingURL=page2a0b94ed8e3fa0369125.js.map